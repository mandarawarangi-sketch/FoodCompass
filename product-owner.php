...
$statement = $db->prepare(
    'SELECT p.id AS product_id,
            s.name, s.review_status, s.review_note, s.submitted_at
     FROM products p
...
$pending = count(array_filter(
    $products,
    fn ($product) => $product['review_status'] === 'pending'
));
$approved = count(array_filter(
    $products,
    fn ($product) => $product['review_status'] === 'approved'
));

function displayText(mixed $value): string
{
...
<title>Product Owner | FoodCompass</title><style>
:root{
...
<div class="container">
    ...
    <div class="grid grid-3">
        <div class="card">
            ...
            <p class="muted">Products managed</p>
        </div>
        <div class="card">
    ...
    <section style="margin-top:26px">
        <table class="table">
            ...
            <tbody>
                <?php if (!$products): ?>
                    <tr>
                        <td colspan="5">You have no product submissions yet.</td>
                    </tr>
                <?php endif; ?>

                <?php foreach ($products as $product): ?>
                    <tr>
                        <td><?= displayText($product['name']) ?></td>
                        <td><?= displayText($product['submitted_at']) ?></td>
                        <td>
                            <span class="status <?= displayText($product['review_status']) ?>">
                                <?= displayText(ucfirst($product['review_status'])) ?>
                            </span>
                        </td>
                        <td><?= displayText($product['review_note'] ?? '—') ?></td>
                        <td>
                            ...
                            <form method="post" action="delete-product.php"
                                ...
                                <input type="hidden" name="delete_token"
                                       value="<?= displayText($_SESSION['delete_token']) ?>">
